...
import { FaRegHeart, FaUserCircle, FaUsers, FaWhatsapp } from "react-icons/fa";
...
const ProgressCounter = () => {
  ...
  const { data: stats = {}, isLoading: statsLoading } = useQuery({
    queryKey: ["progress-info"],
    enabled: !loading,
    queryFn: async () => {
      const res = await axiosPublic.get("/progress-info");
          ...
          <div className="order-1 lg:order-2 space-y-6">
            ...
            <div className="space-y-4 text-gray-600 leading-relaxed">
              <p className="text-base sm:text-lg">
                Track our incredible journey as we continue to bring people
                together. SoulTie has become Bangladesh's trusted platform for
                finding meaningful connections and lifelong partnerships.
              </p>
              <p className="text-base sm:text-lg">
                Every number represents a story of hope, connection, and love.
                Join thousands of satisfied members who found their perfect
        ...
        {statsLoading ? (
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6">
            {statsData?.map((stat, index) => (
              ...
              >
                <div
                  className={`bg-gradient-to-br ${stat.gradient} w-16 h-16 rounded-full flex items-center justify-center mb-4 mx-auto text-white group-hover:rotate-12 transition-transform duration-300`}
                >
                  {stat.icon}
                </div>
